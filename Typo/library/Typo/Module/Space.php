    ...
    protected function stageB()
    {
        ...
        $rules = array(
            ...
            'nbsp' => array(
                ...
                '~(?<=\b)([XIV]{1,5}|[12]\d|[1-9])\h?(?=(вв?|век)\b)~u' => '$1' . $s['nbsp'],
                '~(?<=\b)([1-9]\d{2,3})\h?(?=(год([ауе]|ом)?|гг?\.?)\b)~iu' => '$1' . $s['nbsp'],
                ...
                '~((?:^|\n|[?!.]|' . $s['hellip'] . ')\h?(?:\-{1,3}|' . $s['ndash'] . '))\h~u' => '$1' . $s['nbsp'],

                #B15 Неразрывный пробел перед тире
                '~\h(?=(\-{1,3}|' . $s['ndash'] . ')\h)~u' => $s['nbsp'],
                ...
                '~(?<!' . $s['nbsp'] . ')(?<=\b)([a-zа-яё]{2}|а|в|и|к|о|с|у|я|вне|обо|ото|изо|под|подо|пред|предо|про|над|надо|как|без|безо|что|для|там|ещё|или|меж|между|перед|передо|около|через|сквозь|для|при)\h(?={a}{3,8})~iu' => '$1' . $s['nbsp'],
            ),
            ...
            'thinsp' => array(
                ...
                '~(?<=\b)(\d+)\h?(' . $s['deg'] . ')\h?(?=[CF]\b)~u' => '$1$2' . $s['thinsp'],
            ),
