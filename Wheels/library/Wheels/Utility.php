...
class Utility
{
     ...
     * @return string
     */
    static public function realpath($path, $separator = DIRECTORY_SEPARATOR)
    {
        if (!mb_strlen($path)) {
            return $separator;
        }
        ...
        $win32 = defined('OS_WINDOWS') ? OS_WINDOWS : !strncasecmp(PHP_OS, 'win', 3);
        if ($win32) {
            if (preg_match('/([a-zA-Z]\:)(.*)/', $path, $matches)) {
                ...
                $cwd = getcwd();
                $drive = mb_substr($cwd, 0, 2);
                if ($path{0} !== $separator{0}) {
                    $path = mb_substr($cwd, 3) . $separator . $path;
                }
            }
