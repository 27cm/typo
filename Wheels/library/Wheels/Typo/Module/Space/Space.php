...
class Space extends Module
{
    ...
    public function stageB()
    {
        ...
        $helpers = array(
            // Открывающая кавычка
            '{lquot}' =>
                '(?:"|' . $c['laquo'] . '|' . $c['ldquo'] . '|' . $c['lsquo'] . '|' . $c['bdquo'] . '|' . $c['lsaquo']
                . ')',
        );
        ...
        $rules = array(
            ...
            'nbsp'                               => array(
                ...
                '~(?<=\b)((?:гл|стр|рис|табл|илл?|ст?|п)\.)\h(?=[XIV\d])~iu'                                                                                                                                    =>
                    '$1' . $c['nbsp'],

                #B4 Неразрывный пробел после сокращений г., ул., пер., просп., пл., бул., наб., пр., ш., туп., см., им.
                '~(?<=\b)((?:г|ул|пер|просп|пл|бул|наб|пр|ш|туп|см|им)\.)\h~iu'                                                                                                                                 =>
                    '$1' . $c['nbsp'],
                // Не всегда перед
                // см и им надо оборачивать в nobr
                // test Московский государственный университет им. М. В. Ломоносова

                #B5 Неразрывный пробел после или перед сокращениями б-р, пр-кт
                '~(?<=\b)(б\-р|пр\-кт)\h~iu'                                                                                                                                                                    =>
                    '$1' . $c['nbsp'],
                '~(?<=\b)\h(б\-р|пр\-кт)\b(?!\h)~iu'                                                                                                                                                            =>
                    $c['nbsp'] . '$1',

                #B6 Неразрывный пробел после века и года
                '~(?<=\b)([XIV]{1,5}|[12]\d|[1-9])\h?(?=(вв?|век)\b)~u'                                                                                                                                         =>
                    '$1' . $c['nbsp'],
                '~(?<=\b)([1-9]\d{2,3})\h?(?=(год([ауе]|ом)?|гг?\.?)\b)~iu'                                                                                                                                     =>
                    '$1' . $c['nbsp'],
                ...
                '~([А-ЯЁ]\.)\h([А-ЯЁ]\.)\h(?=[А-ЯЁ][а-яё])~u'                                                                                                                                                   =>
                    '$1' . $c['nbsp'] . '$2' . $c['nbsp'],
                '~([А-ЯЁ]\.)\h(?=[А-ЯЁ][а-яё])~u'                                                                                                                                                               =>
                    '$1' . $c['nbsp'],
                '~([А-ЯЁ][а-яё]+)\h([А-ЯЁ]\.)\h(?=[А-ЯЁ]\.)~u'                                                                                                                                                  =>
                    '$1' . $c['nbsp'] . '$2' . $c['nbsp'],
                '~([А-ЯЁ][а-яё]+)\h(?=[А-ЯЁ]\.)~u'                                                                                                                                                              =>
                    '$1' . $c['nbsp'],

                #B11 Неразрывный пробел в денежных суммах
                '~(?<=\d)\h((?:млн|тыс|млрд)\.?)\h(р(уб)?)(?=\b)~iu'                                                                                                                                            =>
                    $c['nbsp'] . '$1' . $c['nbsp'] . '$2',
                ...
                '~(?<!' . $c['nbsp']
                . ')(?<=\b)([a-zа-яё]{2}|а|в|и|к|о|с|у|я|вне|обо|ото|изо|под|подо|пред|предо|про|над|надо|как|без|безо|что|для|там|ещё|или|меж|между|перед|передо|около|через|сквозь|для|при)\h(?={a}{3,8})~iu' =>
                    '$1' . $c['nbsp'],

                # Неразрывный пробел между сокращением форм собственности и названием организации
                '~(?<=\b)(ООО|ЗАО|ОАО|НИИ|ПБОЮЛ)\h(?={t}*{lquot}?{t}*[A-ZА-ЯЁ])~u'                                                                                                                              =>
                    '$1' . $c['nbsp'],
            ),
